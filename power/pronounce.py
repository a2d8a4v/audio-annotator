...
class PronouncerLex(PronouncerBase):
    ...
    def pronounce(self, words):
        ...
        ## alt_pronounce still cannot process some words
        # prons = [self.lexicon[w] if w in self.lexicon else self.alt_pronounce(
        #     w) for w in wordsLower]
        pattern = r'[0-9]+'
        prons = [self.lexicon[w] if w in self.lexicon else ' '.join([re.sub(pattern, '', p).lower() for p in self.g2p(w.upper())]) for w in wordsLower]
